...
- **Check flags**: physically implausible values (humidity above 200 percent, wind changes of 100 m/s in under 15 seconds, vertical wind beyond 40 m/s) are flagged red as regions, prioritized over gaps, shaded on the graphs, and rolled into every report surface. Users judge each region in its flight context (eyewall penetration vs cruise).
...
- **Legend system**: one checkbox per variable, glowing group chips that swap sensor kinds (direct vs GPS) exclusively, an optional standard deviation band with the whole-flight mean sigma and the worst disagreement moment, and ref linkage chips showing which sensor the ref channel rode at the playhead (click one to jump to its takeover; mid-flight switches get a red badge).
